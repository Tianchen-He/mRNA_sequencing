...
```{r}
exp <- read_xlsx("Data/MazF_1st/Fluc_mRNA_MazFalone_251021_old_charge2_large.xlsx") %>% 
  janitor::clean_names() %>% 
  drop_na() %>% 
...
library(writexl)
write_xlsx(df_matched, "Result/df_matched.xlsx")
...
```{r}
write_xlsx(exp_unmatched,"Result/exp_unmatched.xlsx")
```

# Mass reversion

```{r}
df_comp <- find_compositions_ppm(target_mass = 24479.37129, tolerance = 20, max_methyl = 0, max_length = 120)
```
...
```{r}
write_xlsx(seq_matched,"Result/seq_matched.xlsx")
```
...
highlight_mass <- c(18446.45528,32085.07566,
26042.44436,
29893.87205,
15814.08652,
13800.79178)
