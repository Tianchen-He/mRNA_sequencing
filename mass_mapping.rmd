...
```{r}
df_comp <- find_compositions_ppm(target_mass = 27264.386, tolerance = 10, max_methyl = 0, max_length = 120)
```
...
ggplot() +
  ...
  # Highlight the specific mass in red
  geom_point(
    data = subset(exp, abs(monoisotopic_mass - highlight_mass) < 0.5),
    aes(x = monoisotopic_mass, y = relative_abundance),
  ...
  theme(
    panel.grid = element_blank(),
    legend.position = "none",  # no need for legend since colors are fixed
    axis.line = element_line(color = "black", linewidth = 0.4),
    plot.title = element_text(size = 12, face = "bold", hjust = 0.5)
  ) +
  labs(
